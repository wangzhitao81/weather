...
public class ProvinceHelper {
	...
	private static final String url = "http://www.weather.com.cn/data/city3jdata/china.html";
	
	public Optional<String> getCode(String provinceName) {
		try {
	        ...
	        ResponseEntity<Map<String,String>> resp = restTemplate.exchange(url, HttpMethod.GET, entity, responseType,params);
//			Map<String,String> responseMap = (Map<String, String>) restTemplate.getForEntity(url, ProvinceEntity.class);
	        ...
	        Optional<Entry<String, String>> result = responseMap.entrySet().stream().filter(entry -> entry.getValue().equals(provinceName)).findFirst();
			if(result.isPresent()) {
				//TODO:正常情况下不应该在日志中存在表达式
				logger.info("获取省份编码,{},{}",provinceName,result.get().getKey());
				return Optional.of(result.get().getKey());
			}
		}catch(Exception ex) {
			logger.error("获取省份编码异常",ex);
		}
...
//	private Optional<String> getProvinceCode(String provinceName) {
//		try {
//			HttpHeaders headers = new HttpHeaders();
